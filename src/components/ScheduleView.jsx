...
export default function ScheduleView({ selectedDate, appointments }) {
  ...
  const professionals = [
    { id: '1', name: 'Matías García', title: 'Psicólogo' },
    { id: '2', name: '2', title: 'Sofía Reyes', title: 'Terapeuta Ocupacional' },
  ...
  return (
    <div className="rounded-lg border border-gray-300 bg-white p-6">
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        ...
        <button onClick={goToToday} className="rounded px-3 py-2 text-sm font-medium text-gray-200 bg-gray-600 hover:bg-gray-400 sm:text-base cursor-pointer">
          Hoy
        </button>
        <button onClick={nextDay} className="p-1 text-gray-600 hover:text-blue-600">
          <i className="fas fa-chevron-right text-sm"></i>
        </button>
        <h3 className="text-gray-900 text-sm font-medium sm:text-base">{format(currentDate, 'EEEE, d de MMMM de yyyy', { locale: es })}</h3>
      </div>
      <div className="flex gap-2 sm:gap-3">
        ...
        <button className="rounded px-3 px py-2 bg-blue-600 text-white text-sm font-bold hover:bg-blue-700 sm:text-base cursor-pointer">
    ...
    <div className="overflow-x-auto">
      <div className="min-w-[600px]">
        <div className="grid grid-cols-[80px_repeat(5,1)] gap-2 sm:gap-3">
          <div></div>
        ...
        {timeSlots.map((hour) => (
          <div key={hour} className="grid grid-cols-[80px_repeat(5,1fr)] gap-2 sm:gap-3 border-t pt-2">
                ...
                <div key={prof.id} className="relative min-h-[60px]">
                  {appts.map((appt) => (
                    <button
                      ...
                      className="absolute left-0 right-0 bg-blue-100 text-blue-800 text-xs p-2-sm p-2 hover:bg-blue-200 rounded-md cursor-pointer"
                      style={{
                        top: '0',

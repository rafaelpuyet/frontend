...
export default function Verify() {
  ...
  // Memoize token to ensure stability
  const token = useMemo(() => searchParams.get('token'), [searchParams]);

  useEffect(() => {
    let isMounted = true;
    console.log('useEffect triggered with token:', token); // Debug log

    const verifyToken = async () => {
      if (hasRun) {
        console.log('verifyToken skipped: already run'); // Debug log
        return;
      }
      if (!token) {
        if (isMounted) {
    ...
    return () => {
      isMounted = false;
      console.log('useEffect cleanup'); // Debug log
    };
